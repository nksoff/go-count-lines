...
func TestGetFileName(t *testing.T) {
	f := func(args []string, expectedFileName string, expectError bool) {
		...
		if expectError && err == nil {
			t.Errorf("Expected error, got result string '%s'. Input args: [%s]",
				returnedFileName,
				strings.Join(args, ","))
		} else if !expectError && err != nil {
			t.Errorf("Not expected error, got '%s'. Input args: [%s]",
				err,
				strings.Join(args, ","))
		} else if returnedFileName != expectedFileName {
			t.Errorf("Expected '%s', found '%s'", expectedFileName,
				returnedFileName)
		}
	}

	f([]string{"./count_lines", "123.txt"}, "123.txt", false)
	f([]string{"./count_lines",
		"some/long/long/long/long/long/long/long/long/long/long/file.txt"},
		"some/long/long/long/long/long/long/long/long/long/long/file.txt", false)
...
func TestCountLinesInFile(t*testing.T) {
	f := func(fileName string, expectedLines int, expectError bool) {
		...
		if expectError && err == nil {
			...
			t.Errorf("Not expected error, got '%s'. Input file: %s",
				err,
				fileName)
		} else {
